...
def run_sender_noperm(
    ...
    d = model.dims.activation_d
    d_c = int(np.ceil(d / num_packets))
    d_pad = num_packets * d_c
    ...
    frame_processed_total = 0.0

    patterns = np.load("interleave_patterns.npy")
    ...
    while True:
        ...
        choose = random.randint(0, total_patterns - 1)
        send_order = patterns[choose].tolist()
        # print(f"Send order for frame {frame_id}: {send_order}")

        n_sent = 0

        if act.shape[0] != d:
            raise ValueError(f"Expected activation of length {d}, got {act.shape[0]}")
        if d_pad > d:
            packet_serial = np.pad(act, (0, d_pad - d))

        packet = packet_serial.reshape(num_packets, d_c)

        loop_start_time = time.perf_counter()
        for idx in send_order:
            payload = packet[idx].astype(np.float16, copy=False).tobytes()
            header = struct.pack(HEADER_FMT, frame_id, idx, time.time())
            sock.sendto(header + payload, (host, port))
            n_sent += 1
        send_loop_ms = (time.perf_counter() - loop_start_time) * 1e3
        ...
        while True:
            ...
            if int(msg.get("frame_id", -1)) != frame_id and int(msg.get("frame_id", -1)) not in seen_out_of_order:
                ...
                print(f"Received response for frame {msg.get('frame_id')} while waiting for frame {frame_id}, ignoring.")
                continue
            result = msg
        ...
        frame_processed_time = (time.perf_counter() - frame_taken)*1e3
        print(f"Frame {frame_id} processed in {frame_processed_time:.2f} ms.")
        ...
        if result is None:
            ...
            frame_processed_total += frame_processed_time
            ...
            detections_by_frame[frame_id - 1] = np.array(recv_detections, dtype=np.float32) if recv_detections else np.zeros((0, 6), dtype=np.float32)
            ...
            cv2.putText(
                annotated,
                f"enc {enc_ms:.1f}ms tx {tx_ms:.1f}ms frame_proc {frame_processed_time:.1f}ms dec {decode_ms:.1f}ms e2e {e2e_ms:.1f}ms",
                (10, 56),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.55,
                (0, 255, 255),
            ...
            print(
                ...
                f"frame_proc={frame_processed_time:6.2f}ms  dec={decode_ms:6.2f}ms  "
    ...
    avg_frame_processed = frame_processed_total / max(processed - timeout_total, 1)
    ...
    print(f"  Avg total process time : {avg_frame_processed:.2f} ms")
    ...
    print(f"recv_total: {recv_total}, num_packets: {num_packets}")
